...
from trustllm_pkg.trustllm.utils.generation_utils import *
...
class LLMGeneration:
    def __init__(self,
        ...
        self.debug = debug
        # Read mapping directly from this repo's config to avoid picking up a pip-installed package.
        self.model_mapping = config.model_info["model_mapping"]
        self.online_model_list = config.model_info["online_model"]
        if not device:
    ...
    def generation(self, model_name, prompt, tokenizer, model, temperature=None):
        ...
        try:
            if model_name in self.online_model_list and self.online_model: # and (self.use_replicate or self.use_deepinfra): <-- IGNORE ---
                ans = gen_online(
                    model_name,
                    prompt,
                    temperature,
    ...
    def process_file(self, data_path, save_path, model_name, tokenizer, model, file_config, key_name='prompt'):
        ...
        # Resume from previous run if possible; fall back to source data if the cache is corrupted
        if os.path.exists(save_path):
            try:
                with open(save_path, 'r', encoding='utf-8') as f:
                    saved_data = json.load(f)
            except (json.JSONDecodeError, UnicodeDecodeError) as exc:
                print(f"Warning: could not read existing save file {save_path} ({exc}); restarting this file.")
                saved_data = list(original_data)
    ...
    def _run_task(self, model_name, model, tokenizer, base_dir, file_config, key_name='prompt'):
        ...
        for file in tqdm(file_list, desc="Processing files"):
            ...
            model_name = self.model_mapping.get(self.model_path, "")
            save_path = os.path.join('generation_results', self.model_name, section, file)
    ...
    def _run_single_test(self):
        ...
        if (model_name in self.online_model_list) and ((self.online_model or self.use_replicate) or (self.online_model or self.use_deepinfra)):
