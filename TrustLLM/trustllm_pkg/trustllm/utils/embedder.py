...
class DataEmbedder:
    ...
    def get_embeddings(self, string, embedding_model='text-embedding-ada-002', ):
        ...
        if config.azure_openai:
            ...
            model = config.azure_embedding_engine
            client = AzureOpenAI(
            ...
            if config.openai_api_base is not None:
                ...
                client = OpenAI(api_key=api_key, base_url=config.openai_api_base, )
                response = client.embeddings.create(
    ...
    def parallel_embedding(self, data, embedding_func, filename):
        with ThreadPoolExecutor(max_workers=config.max_worker_embedding) as executor:
