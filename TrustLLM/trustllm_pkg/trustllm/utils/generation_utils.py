...
def get_res_openai(string, model, temperature, max_tokens=None):
    gpt_model_mapping = {
        "chatgpt": "gpt-3.5-turbo",
        "gpt-4": "gpt-4-1106-preview",
        "llama-3-8b": "llama-3-8b",  # local vLLM served name
        "qwen-2.5-7B": "qwen-2.5-7B",
        "llama-3-8b-wanda-0.5": "llama-3-8b-wanda-0.5",
        "llama-3-8b-sparsegpt-0.5": "llama-3-8b-sparsegpt-0.5",
    }
    gpt_model = gpt_model_mapping.get(model, model)
    api_key = config.openai_key
    client = OpenAI(api_key=api_key, base_url=getattr(config, "openai_api_base", None))
    response = client.chat.completions.create(
        model=gpt_model,
        messages=[{"role": "user", "content": string}],
        temperature=temperature,
        # Respect caller-specified cap; OpenAI API calls this max_tokens.
...
def deepinfra_api(string, model, temperature, max_tokens=None):
...
def claude_api(string, model, temperature):
    anthropic = Anthropic(api_key=config.claude_api)
...
def gen_online(model_name, prompt, temperature, max_new_tokens=None, replicate=False, deepinfra=False):
    if model_name in model_info['wenxin_model']:
        ...
        res = get_res_openai(prompt, model=model_name, temperature=temperature, max_tokens=max_new_tokens)
    elif model_name in model_info['deepinfra_model']:
        res = deepinfra_api(prompt, model=model_name, temperature=temperature, max_tokens=max_new_tokens)
        ...
        res = deepinfra_api(prompt, model_name, temperature, max_tokens=max_new_tokens)
    else:
