...
def get_res(string, model='gpt-4o', temperature=0, message=None):
    """
    Retrieve a response from the OpenAI ChatCompletion API.

    Args:
        string (str): The input string to process.
        model (str): The model to use for generating the response. Default is 'gpt-5-nano'.
    ...
    """
    try:
        # Some OpenAI-compatible endpoints reject temperature=0; fall back to 1 (default) in that case.
        api_temperature = 1 if temperature == 0 else temperature
        if message is None:
